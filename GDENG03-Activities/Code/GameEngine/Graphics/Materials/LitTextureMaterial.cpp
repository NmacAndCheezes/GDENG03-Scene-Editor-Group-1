...
LitTextureMaterial::LitTextureMaterial(std::string textureName) : AMaterial(L"LitTextureShader"), textureName(textureName)
{
...
void LitTextureMaterial::RenderUI()
{
    ...
    if (ImGui::ImageButton("#AlbedoButton", (ImTextureID)(intptr_t)texture->GetTextureView(), ImVec2(50.f, 50.f)))
    {
        const char* filters =  
            ...
            "JPEG (*.jpeg)\0*.jpeg\0"
        ;
        std::string file_path = FileDialogs::OpenFile(filters);
...
std::string LitTextureMaterial::GetTextureName()
{
    return textureName;
}
